...
export async function purchaseReservation(reservationId, userId) {
  ...
  if (userId === undefined || userId === null || isNaN(parsedUserId)) {
    const err = new Error(
      "Purchase requires a valid userId in the request body",
    );
    err.status = 400;
    throw err;
  }
  ...
  return await db
    .transaction(async (tx) => {
      ...
      const now = new Date();
      const purchased = await tx.orm.public.Reservation.where({
        id: parsedReservationId,
        status: "ACTIVE",
      })
      ...
      if (!purchased) {
        const current = await tx.orm.public.Reservation.where({
          id: parsedReservationId,
        }).first();
        if (!current) {
          const err = new Error("Reservation not found");
